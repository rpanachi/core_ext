...
class KeyGeneratorTest < CoreExt::TestCase
  def setup
    @secret    = SecureRandom.hex(64)
    @generator = CoreExt::KeyGenerator.new(@secret, :iterations=>2)
  end
...
class CachingKeyGeneratorTest < CoreExt::TestCase
  def setup
    @secret    = SecureRandom.hex(64)
    @generator = CoreExt::KeyGenerator.new(@secret, :iterations=>2)
    @caching_generator = CoreExt::CachingKeyGenerator.new(@generator)
  end
