...
class AlsoDoingNothingTest < CoreExt::TestCase
end

# Setup and teardown callbacks.
class SetupAndTeardownTest < CoreExt::TestCase
...
class TestCaseTaggedLoggingTest < CoreExt::TestCase
  def before_setup
    require 'stringio'
    @out = StringIO.new
    self.tagged_logger = CoreExt::TaggedLogging.new(Logger.new(@out))
    super
  end
...
class TestOrderTest < CoreExt::TestCase
  def setup
    @original_test_order = CoreExt::TestCase.test_order
  end

  def teardown
    CoreExt::TestCase.test_order = @original_test_order
  end

  def test_defaults_to_random
    CoreExt::TestCase.test_order = nil

    assert_equal :random, CoreExt::TestCase.test_order

    assert_equal :random, CoreExt.test_order
  end

  def test_test_order_is_global
    CoreExt::TestCase.test_order = :sorted

    assert_equal :sorted, CoreExt.test_order
    assert_equal :sorted, CoreExt::TestCase.test_order
    assert_equal :sorted, self.class.test_order
    assert_equal :sorted, Class.new(CoreExt::TestCase).test_order
    ...
    assert_equal :random, Class.new(CoreExt::TestCase).test_order
  end
end
