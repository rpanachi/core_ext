...
class MessageEncryptorTest < CoreExt::TestCase
  class JSONSerializer
    def dump(value)
      CoreExt::JSON.encode(value)
    end

    def load(value)
      CoreExt::JSON.decode(value)
    end
  end

  def setup
    @secret    = SecureRandom.hex(64)
    @verifier  = CoreExt::MessageVerifier.new(@secret, :serializer => CoreExt::MessageEncryptor::NullSerializer)
    @encryptor = CoreExt::MessageEncryptor.new(@secret)
  ...
  def test_alternative_serialization_method
    ...
    CoreExt.use_standard_json_time_format = true
    encryptor = CoreExt::MessageEncryptor.new(SecureRandom.hex(64), SecureRandom.hex(64), :serializer => JSONSerializer.new)
  ...
  def assert_not_decrypted(value)
    assert_raise(CoreExt::MessageEncryptor::InvalidMessage) do
      @encryptor.decrypt_and_verify(@verifier.generate(value))
    end
  end

  def assert_not_verified(value)
    assert_raise(CoreExt::MessageVerifier::InvalidSignature) do
