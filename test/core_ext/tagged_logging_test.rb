...
class TaggedLoggingTest < CoreExt::TestCase
  class MyLogger < ::CoreExt::Logger
    def flush(*)
      info "[FLUSHED]"
    end
  end
  ...
  test "keeps each tag in their own instance" do
    ...
    @other_logger = CoreExt::TaggedLogging.new(MyLogger.new(@other_output))
