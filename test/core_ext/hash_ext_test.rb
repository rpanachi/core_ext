...
class HashExtTest < CoreExt::TestCase
  class IndifferentHash < CoreExt::HashWithIndifferentAccess
  end
  ...
  def test_stringify_keys_for_hash_with_indifferent_access
    assert_instance_of CoreExt::HashWithIndifferentAccess, @symbols.with_indifferent_access.stringify_keys
  ...
  def test_deep_stringify_keys_for_hash_with_indifferent_access
    assert_instance_of CoreExt::HashWithIndifferentAccess, @nested_symbols.with_indifferent_access.deep_stringify_keys
  ...
  def test_stringify_keys_bang_for_hash_with_indifferent_access
    assert_instance_of CoreExt::HashWithIndifferentAccess, @symbols.with_indifferent_access.dup.stringify_keys!
  ...
  def test_deep_stringify_keys_bang_for_hash_with_indifferent_access
    assert_instance_of CoreExt::HashWithIndifferentAccess, @nested_symbols.with_indifferent_access.dup.deep_stringify_keys!
  ...
  def test_nested_under_indifferent_access
    ...
    assert_kind_of CoreExt::HashWithIndifferentAccess, foo["foo"]
  ...
  def test_indifferent_select
    hash = CoreExt::HashWithIndifferentAccess.new(@strings).select {|k,v| v == 1}

    assert_equal({ 'a' => 1 }, hash)
    assert_instance_of CoreExt::HashWithIndifferentAccess, hash
  end

  def test_indifferent_select_returns_enumerator
    enum = CoreExt::HashWithIndifferentAccess.new(@strings).select
    assert_instance_of Enumerator, enum
  end

  def test_indifferent_select_returns_a_hash_when_unchanged
    hash = CoreExt::HashWithIndifferentAccess.new(@strings).select {|k,v| true}

    assert_instance_of CoreExt::HashWithIndifferentAccess, hash
  end

  def test_indifferent_select_bang
    indifferent_strings = CoreExt::HashWithIndifferentAccess.new(@strings)
    indifferent_strings.select! {|k,v| v == 1}

    assert_equal({ 'a' => 1 }, indifferent_strings)
    assert_instance_of CoreExt::HashWithIndifferentAccess, indifferent_strings
  end

  def test_indifferent_reject
    hash = CoreExt::HashWithIndifferentAccess.new(@strings).reject {|k,v| v != 1}
  ...
  def test_indifferent_reject_returns_enumerator
    enum = CoreExt::HashWithIndifferentAccess.new(@strings).reject
    assert_instance_of Enumerator, enum
  end
...
class HashExtToParamTests < CoreExt::TestCase
...
class HashToXmlTest < CoreExt::TestCase
  def setup
  ...
  def test_from_xml_raises_on_disallowed_type_attributes
    assert_raise CoreExt::XMLConverter::DisallowedType do
  ...
  def test_expansion_count_is_limited
    expected =
      case CoreExt::XmlMini.backend.name
      when 'CoreExt::XmlMini_REXML';        RuntimeError
      when 'CoreExt::XmlMini_Nokogiri';     Nokogiri::XML::SyntaxError
      when 'CoreExt::XmlMini_NokogiriSAX';  RuntimeError
      when 'CoreExt::XmlMini_LibXML';       LibXML::XML::Error
      when 'CoreExt::XmlMini_LibXMLSAX';    LibXML::XML::Error
      end
