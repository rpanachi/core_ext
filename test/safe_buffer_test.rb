...
class SafeBufferTest < CoreExt::TestCase
  ...
  test "Should return a safe buffer when calling to_s" do
    ...
    assert_equal CoreExt::SafeBuffer, new_buffer.class
  end

  test "Should be converted to_yaml" do
    str  = 'hello!'
    buf  = CoreExt::SafeBuffer.new str
    yaml = buf.to_yaml
  ...
  test "Should work in nested to_yaml conversion" do
    str  = 'hello!'
    data = { 'str' => CoreExt::SafeBuffer.new(str) }
    yaml = YAML.dump data
    assert_equal({'str' => str}, YAML.load(yaml))
  end

  test "Should work with primitive-like-strings in to_yaml conversion" do
    assert_equal 'true',  YAML.load(CoreExt::SafeBuffer.new('true').to_yaml)
    assert_equal 'false', YAML.load(CoreExt::SafeBuffer.new('false').to_yaml)
    assert_equal '1',     YAML.load(CoreExt::SafeBuffer.new('1').to_yaml)
    assert_equal '1.1',   YAML.load(CoreExt::SafeBuffer.new('1.1').to_yaml)
  end
  ...
  test "Should raise an error when safe_concat is called on unsafe buffers" do
    @buffer.gsub!('', '<>')
    assert_raise CoreExt::SafeBuffer::SafeConcatError do
  ...
  test "clone_empty returns an empty buffer" do
    assert_equal '', CoreExt::SafeBuffer.new('foo').clone_empty
  end
