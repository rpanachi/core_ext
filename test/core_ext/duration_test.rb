...
class DurationTest < CoreExt::TestCase
  include TimeZoneTestHelpers

  def test_is_a
    d = 1.day
    assert d.is_a?(CoreExt::Duration)
    assert_kind_of CoreExt::Duration, d
  ...
  def test_instance_of
    ...
    assert 2.days.instance_of?(CoreExt::Duration)
    assert !3.second.instance_of?(Numeric)
  end

  def test_threequals
    assert CoreExt::Duration === 1.day
    assert !(CoreExt::Duration === 1.day.to_i)
    assert !(CoreExt::Duration === 'foo')
  end
  ...
  def test_since_and_ago_anchored_to_time_now_when_time_zone_is_not_set
    ...
    with_env_tz 'US/Eastern' do
      Time.stub(:now, Time.local(2000)) do
        # since
        assert_not_instance_of CoreExt::TimeWithZone, 5.seconds.since
        assert_equal Time.local(2000,1,1,0,0,5), 5.seconds.since
        # ago
        assert_not_instance_of CoreExt::TimeWithZone, 5.seconds.ago
  ...
  def test_since_and_ago_anchored_to_time_zone_now_when_time_zone_is_set
    ...
    with_env_tz 'US/Eastern' do
      Time.stub(:now, Time.local(2000)) do
        # since
        assert_instance_of CoreExt::TimeWithZone, 5.seconds.since
        ...
        # ago
        assert_instance_of CoreExt::TimeWithZone, 5.seconds.ago
