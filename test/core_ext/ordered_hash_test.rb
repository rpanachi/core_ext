...
class OrderedHashTest < CoreExt::TestCase
  def setup
    @keys =   %w( blue   green  red    pink   orange )
    @values = %w( 000099 009900 aa0000 cc0066 cc6633 )
    @hash = Hash.new
    @ordered_hash = CoreExt::OrderedHash.new
  ...
  def test_select
    ...
    assert_instance_of CoreExt::OrderedHash, new_ordered_hash
  end
  ...
  def test_merge
    other_hash =  CoreExt::OrderedHash.new
  ...
  def test_alternate_initialization_raises_exception_on_odd_length_args
    assert_raises ArgumentError do
      CoreExt::OrderedHash[1,2,3,4,5]
    end
  end

  def test_replace_updates_keys
    @other_ordered_hash = CoreExt::OrderedHash[:black, '000000', :white, '000000']
  ...
  def test_nested_under_indifferent_access
    flash = {:a => CoreExt::OrderedHash[:b, 1, :c, 2]}.with_indifferent_access
    assert_kind_of CoreExt::OrderedHash, flash[:a]
  end
  ...
  def test_update_sets_keys
    @updated_ordered_hash = CoreExt::OrderedHash.new
    @updated_ordered_hash.update(:name => "Bob")
    assert_equal [:name],  @updated_ordered_hash.keys
  end

  def test_invert
    expected = CoreExt::OrderedHash[@values.zip(@keys)]
