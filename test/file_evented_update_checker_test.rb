...
class FileEventedUpdateCheckerTest < CoreExt::TestCase
  include FileUpdateCheckerSharedTests

  def new_checker(files = [], dirs = {}, &block)
    CoreExt::FileEventedUpdateChecker.new(files, dirs, &block).tap do
      wait
    end
  end
...
class FileEventedUpdateCheckerPathHelperTest < CoreExt::TestCase
  def pn(path)
    Pathname.new(path)
  end

  setup do
    @ph = CoreExt::FileEventedUpdateChecker::PathHelper.new
  end
