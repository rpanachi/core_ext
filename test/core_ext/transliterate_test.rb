...
class TransliterateTest < CoreExt::TestCase
  def test_transliterate_should_not_change_ascii_chars
    (0..127).each do |byte|
      char = [byte].pack("U")
      assert_equal char, CoreExt::Inflector.transliterate(char)
    end
  end
  ...
  def test_transliterate_should_approximate_ascii
    ...
    string.each_char do |char|
      assert_match %r{^[a-zA-Z']*$}, CoreExt::Inflector.transliterate(char)
    end
  end
  ...
  def test_transliterate_should_work_with_custom_i18n_rules_and_uncomposed_utf8
    ...
    assert_equal "ue", CoreExt::Inflector.transliterate(char)
  ensure
    I18n.locale = default_locale
  end

  def test_transliterate_should_allow_a_custom_replacement_char
    assert_equal "a*b", CoreExt::Inflector.transliterate("a索b", "*")
  end
end
