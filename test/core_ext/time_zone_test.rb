...
class TimeZoneTest < CoreExt::TestCase
  include TimeZoneTestHelpers

  def test_utc_to_local
    zone = CoreExt::TimeZone['Eastern Time (US & Canada)']
  ...
  CoreExt::TimeZone::MAPPING.each_key do |name|
    define_method("test_map_#{name.downcase.gsub(/[^a-z]/, '_')}_to_tzinfo") do
      zone = CoreExt::TimeZone[name]
      assert_respond_to zone.tzinfo, :period_for_local
    end
  end

  def test_from_integer_to_map
    assert_instance_of CoreExt::TimeZone, CoreExt::TimeZone[-28800] # PST
  end

  def test_from_duration_to_map
    assert_instance_of CoreExt::TimeZone, CoreExt::TimeZone[-480.minutes] # PST
  end

  CoreExt::TimeZone.all.each do |zone|
    name = zone.name.downcase.gsub(/[^a-z]/, '_')
    define_method("test_from_#{name}_to_map") do
      assert_instance_of CoreExt::TimeZone, CoreExt::TimeZone[zone.name]
    end
  ...
  def test_now
    with_env_tz 'US/Eastern' do
      zone = CoreExt::TimeZone['Eastern Time (US & Canada)'].dup
      def zone.time_now; Time.local(2000); end
      assert_instance_of CoreExt::TimeWithZone, zone.now
  ...
  def test_unknown_timezones_delegation_to_tzinfo
    zone = CoreExt::TimeZone['America/Montevideo']
    assert_equal CoreExt::TimeZone, zone.class
    assert_equal zone.object_id, CoreExt::TimeZone['America/Montevideo'].object_id
  ...
  def test_today
    ...
    assert_equal Date.new(1999, 12, 31), CoreExt::TimeZone['Eastern Time (US & Canada)'].today
    travel_to(Time.utc(2000, 1, 1, 5)) # midnight Jan 1 EST
    assert_equal Date.new(2000, 1, 1), CoreExt::TimeZone['Eastern Time (US & Canada)'].today
    travel_to(Time.utc(2000, 1, 2, 4, 59, 59)) # 1 sec before midnight Jan 2 EST
    assert_equal Date.new(2000, 1, 1), CoreExt::TimeZone['Eastern Time (US & Canada)'].today
    travel_to(Time.utc(2000, 1, 2, 5)) # midnight Jan 2 EST
    assert_equal Date.new(2000, 1, 2), CoreExt::TimeZone['Eastern Time (US & Canada)'].today
    travel_back
  end

  def test_tomorrow
    travel_to(Time.utc(2000, 1, 1, 4, 59, 59)) # 1 sec before midnight Jan 1 EST
    assert_equal Date.new(2000, 1, 1), CoreExt::TimeZone['Eastern Time (US & Canada)'].tomorrow
    travel_to(Time.utc(2000, 1, 1, 5)) # midnight Jan 1 EST
    assert_equal Date.new(2000, 1, 2), CoreExt::TimeZone['Eastern Time (US & Canada)'].tomorrow
    travel_to(Time.utc(2000, 1, 2, 4, 59, 59)) # 1 sec before midnight Jan 2 EST
    assert_equal Date.new(2000, 1, 2), CoreExt::TimeZone['Eastern Time (US & Canada)'].tomorrow
    travel_to(Time.utc(2000, 1, 2, 5)) # midnight Jan 2 EST
    assert_equal Date.new(2000, 1, 3), CoreExt::TimeZone['Eastern Time (US & Canada)'].tomorrow
    travel_back
  end

  def test_yesterday
    travel_to(Time.utc(2000, 1, 1, 4, 59, 59)) # 1 sec before midnight Jan 1 EST
    assert_equal Date.new(1999, 12, 30), CoreExt::TimeZone['Eastern Time (US & Canada)'].yesterday
    travel_to(Time.utc(2000, 1, 1, 5)) # midnight Jan 1 EST
    assert_equal Date.new(1999, 12, 31), CoreExt::TimeZone['Eastern Time (US & Canada)'].yesterday
    travel_to(Time.utc(2000, 1, 2, 4, 59, 59)) # 1 sec before midnight Jan 2 EST
    assert_equal Date.new(1999, 12, 31), CoreExt::TimeZone['Eastern Time (US & Canada)'].yesterday
    travel_to(Time.utc(2000, 1, 2, 5)) # midnight Jan 2 EST
    assert_equal Date.new(2000, 1, 1), CoreExt::TimeZone['Eastern Time (US & Canada)'].yesterday
    travel_back
  end
  ...
  def test_local
    time = CoreExt::TimeZone["Hawaii"].local(2007, 2, 5, 15, 30, 45)
    assert_equal Time.utc(2007, 2, 5, 15, 30, 45), time.time
    assert_equal CoreExt::TimeZone["Hawaii"], time.time_zone
  end

  def test_local_with_old_date
    time = CoreExt::TimeZone["Hawaii"].local(1850, 2, 5, 15, 30, 45)
  ...
  def test_parse_string_with_timezone
    (-11..13).each do |timezone_offset|
      zone = CoreExt::TimeZone[timezone_offset]
      twz = zone.parse('1999-12-31 19:00:00')
  ...
  def test_parse_should_not_black_out_system_timezone_dst_jump
    with_env_tz('EET') do
      zone = CoreExt::TimeZone['Pacific Time (US & Canada)']
  ...
  def test_strptime_with_nondefault_time_zone
    with_tz_default CoreExt::TimeZone['Pacific Time (US & Canada)'] do
      zone = CoreExt::TimeZone['Eastern Time (US & Canada)']
      twz = zone.strptime('1999-12-31 12:00:00', '%Y-%m-%d %H:%M:%S')
      assert_equal Time.utc(1999,12,31,17), twz
      assert_equal Time.utc(1999,12,31,12), twz.time
  ...
  def test_utc_offset_lazy_loaded_from_tzinfo_when_not_passed_in_to_initialize
    ...
    zone = CoreExt::TimeZone.create(tzinfo.name, nil, tzinfo)
  ...
  def test_seconds_to_utc_offset_with_colon
    assert_equal "-06:00", CoreExt::TimeZone.seconds_to_utc_offset(-21_600)
    assert_equal "+00:00", CoreExt::TimeZone.seconds_to_utc_offset(0)
    assert_equal "+05:00", CoreExt::TimeZone.seconds_to_utc_offset(18_000)
  end

  def test_seconds_to_utc_offset_without_colon
    assert_equal "-0600", CoreExt::TimeZone.seconds_to_utc_offset(-21_600, false)
    assert_equal "+0000", CoreExt::TimeZone.seconds_to_utc_offset(0, false)
    assert_equal "+0500", CoreExt::TimeZone.seconds_to_utc_offset(18_000, false)
  end

  def test_seconds_to_utc_offset_with_negative_offset
    assert_equal "-01:00", CoreExt::TimeZone.seconds_to_utc_offset(-3_600)
    assert_equal "-00:59", CoreExt::TimeZone.seconds_to_utc_offset(-3_599)
    assert_equal "-05:30", CoreExt::TimeZone.seconds_to_utc_offset(-19_800)
  end

  def test_formatted_offset_positive
    zone = CoreExt::TimeZone['New Delhi']
  ...
  def test_formatted_offset_zero
    zone = CoreExt::TimeZone['London']
    assert_equal "+00:00", zone.formatted_offset
    assert_equal "UTC", zone.formatted_offset(true, 'UTC')
  end

  def test_zone_compare
    zone1 = CoreExt::TimeZone['Central Time (US & Canada)'] # offset -0600
    zone2 = CoreExt::TimeZone['Eastern Time (US & Canada)'] # offset -0500
    assert zone1 < zone2
    assert zone2 > zone1
    assert zone1 == zone1
  end
  ...
  def test_to_s
    assert_equal "(GMT+05:30) New Delhi", CoreExt::TimeZone['New Delhi'].to_s
  end

  def test_all_sorted
    all = CoreExt::TimeZone.all
  ...
  def test_index
    assert_nil CoreExt::TimeZone["bogus"]
    assert_instance_of CoreExt::TimeZone, CoreExt::TimeZone["Central Time (US & Canada)"]
    assert_instance_of CoreExt::TimeZone, CoreExt::TimeZone[8]
    assert_raise(ArgumentError) { CoreExt::TimeZone[false] }
  end

  def test_unknown_zone_raises_exception
    assert_raise TZInfo::InvalidTimezoneIdentifier do
      CoreExt::TimeZone.create("bogus")
    end
  end
  ...
  def test_new
    assert_equal CoreExt::TimeZone["Central Time (US & Canada)"], CoreExt::TimeZone.new("Central Time (US & Canada)")
  end

  def test_us_zones
    assert CoreExt::TimeZone.us_zones.include?(CoreExt::TimeZone["Hawaii"])
    assert !CoreExt::TimeZone.us_zones.include?(CoreExt::TimeZone["Kuala Lumpur"])
  end

  def test_to_yaml
    assert_equal("--- !ruby/object:CoreExt::TimeZone\nname: Pacific/Honolulu\n", CoreExt::TimeZone["Hawaii"].to_yaml)
    assert_equal("--- !ruby/object:CoreExt::TimeZone\nname: Europe/London\n", CoreExt::TimeZone["Europe/London"].to_yaml)
  end

  def test_yaml_load
    assert_equal(CoreExt::TimeZone["Pacific/Honolulu"], YAML.load("--- !ruby/object:CoreExt::TimeZone\nname: Pacific/Honolulu\n"))
  end
end
