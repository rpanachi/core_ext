...
require 'core_ext/builder'
require 'core_ext/hash'
require 'core_ext/big_decimal'
require 'yaml'

module XmlMiniTest
  class RenameKeyTest < CoreExt::TestCase
    def test_rename_key_dasherizes_by_default
      assert_equal "my-key", CoreExt::XmlMini.rename_key("my_key")
    end

    def test_rename_key_dasherizes_with_dasherize_true
      assert_equal "my-key", CoreExt::XmlMini.rename_key("my_key", :dasherize => true)
    end

    def test_rename_key_does_nothing_with_dasherize_false
      assert_equal "my_key", CoreExt::XmlMini.rename_key("my_key", :dasherize => false)
    end

    def test_rename_key_camelizes_with_camelize_true
      assert_equal "MyKey", CoreExt::XmlMini.rename_key("my_key", :camelize => true)
    end

    def test_rename_key_lower_camelizes_with_camelize_lower
      assert_equal "myKey", CoreExt::XmlMini.rename_key("my_key", :camelize => :lower)
    end

    def test_rename_key_lower_camelizes_with_camelize_upper
      assert_equal "MyKey", CoreExt::XmlMini.rename_key("my_key", :camelize => :upper)
    end

    def test_rename_key_does_not_dasherize_leading_underscores
      assert_equal "_id", CoreExt::XmlMini.rename_key("_id")
    end

    def test_rename_key_with_leading_underscore_dasherizes_interior_underscores
      assert_equal "_my-key", CoreExt::XmlMini.rename_key("_my_key")
    end

    def test_rename_key_does_not_dasherize_trailing_underscores
      assert_equal "id_", CoreExt::XmlMini.rename_key("id_")
    end

    def test_rename_key_with_trailing_underscore_dasherizes_interior_underscores
      assert_equal "my-key_", CoreExt::XmlMini.rename_key("my_key_")
    end

    def test_rename_key_does_not_dasherize_multiple_leading_underscores
      assert_equal "__id", CoreExt::XmlMini.rename_key("__id")
    end

    def test_rename_key_does_not_dasherize_multiple_trailing_underscores
      assert_equal "id__", CoreExt::XmlMini.rename_key("id__")
    end
  end

  class ToTagTest < CoreExt::TestCase
    def assert_xml(xml)
  ...
  class WithBackendTest < CoreExt::TestCase
    module REXML end
    module LibXML end
    module Nokogiri end

    setup do
      @xml, @default_backend = CoreExt::XmlMini, CoreExt::XmlMini.backend
    end

    teardown do
      CoreExt::XmlMini.backend = @default_backend
    end
  ...
  class ThreadSafetyTest < CoreExt::TestCase
    module REXML end
    module LibXML end
  ...
  class ParsingTest < CoreExt::TestCase
    def setup
      @parsing = CoreExt::XmlMini::PARSING
    end
