...
class ToSentenceTest < CoreExt::TestCase
  ...
  def test_always_returns_string
    assert_instance_of String, [CoreExt::SafeBuffer.new('one')].to_sentence
    assert_instance_of String, [CoreExt::SafeBuffer.new('one'), 'two'].to_sentence
    assert_instance_of String, [CoreExt::SafeBuffer.new('one'), 'two', 'three'].to_sentence
  end
end

class ToSTest < CoreExt::TestCase
  class TestDB
    @@counter = 0
    def id
      @@counter += 1
...
class ToXmlTest < CoreExt::TestCase
