...
require 'core_ext/testing/stream'

class Deprecatee
  def initialize
    @request = CoreExt::Deprecation::DeprecatedInstanceVariableProxy.new(self, :request)
  ...
  def partially(foo = nil)
    CoreExt::Deprecation.warn('calling with foo=nil is out') if foo.nil?
  end
  ...
  A = CoreExt::Deprecation::DeprecatedConstantProxy.new('Deprecatee::A', 'Deprecatee::B::C')
end


class DeprecationTest < CoreExt::TestCase
  include CoreExt::Testing::Stream

  def setup
    # Track the last warning.
    @old_behavior = CoreExt::Deprecation.behavior
    @last_message = nil
    CoreExt::Deprecation.behavior = Proc.new { |message| @last_message = message }

    @dtc = Deprecatee.new
  end

  def teardown
    CoreExt::Deprecation.behavior = @old_behavior
  end
  ...
  def test_deprecate_object
    deprecated_object = CoreExt::Deprecation::DeprecatedObjectProxy.new(Object.new, ':bomb:')
    assert_deprecated(/:bomb:/) { deprecated_object.to_s }
  end

  def test_nil_behavior_is_ignored
    CoreExt::Deprecation.behavior = nil
  ...
  def test_raise_behaviour
    CoreExt::Deprecation.behavior = :raise

    message   = 'Revise this deprecated stuff now!'
    callstack = %w(foo bar baz)

    e = assert_raise CoreExt::DeprecationException do
      CoreExt::Deprecation.behavior.first.call(message, callstack)
    end
  ...
  def test_default_stderr_behavior
    ...
    behavior = CoreExt::Deprecation.behavior.first
  ...
  def test_default_stderr_behavior_with_warn_method
    ...
    content = capture(:stderr) {
      CoreExt::Deprecation.warn('Instance error!', ['instance call stack!'])
    }
  ...
  def test_default_silence_behavior
    CoreExt::Deprecation.behavior = :silence
    behavior = CoreExt::Deprecation.behavior.first
  ...
  def test_assert_deprecated_matches_any_warning
    assert_deprecated 'abc' do
      CoreExt::Deprecation.warn 'abc'
      CoreExt::Deprecation.warn 'def'
    end
  ...
  def test_assert_deprecated_warn_work_with_default_behavior
    CoreExt::Deprecation.instance_variable_set('@behavior' , nil)
    assert_deprecated('abc') do
      CoreExt::Deprecation.warn 'abc'
    end
  end
  ...
  def test_silence
    ...
    CoreExt::Deprecation.silenced = true
    assert_not_deprecated { @dtc.partially }
    CoreExt::Deprecation.silenced = false
  end
  ...
  def test_deprecation_in_other_object
    ...
    klass = Class.new do
      delegate :warn, :behavior=, to: CoreExt::Deprecation
    end
  ...
  def test_deprecated_constant_with_deprecator_given
    deprecator = deprecator_with_messages
    klass = Class.new
    klass.const_set(:OLD, CoreExt::Deprecation::DeprecatedConstantProxy.new('klass::OLD', 'Object', deprecator) )
  ...
  def test_deprecated_instance_variable_with_instance_deprecator
    ...
    klass = Class.new() do
      def initialize(deprecator)
        @request = CoreExt::Deprecation::DeprecatedInstanceVariableProxy.new(self, :request, :@request, deprecator)
        @_request = :a_request
      end
      def request; @_request end
      def old_request; @request end
  ...
  def test_default_gem_name
    deprecator = CoreExt::Deprecation.new
  ...
  def test_custom_gem_name
    deprecator = CoreExt::Deprecation.new('2.0', 'Custom')
  ...
  private
    def deprecator_with_messages
      klass = Class.new(CoreExt::Deprecation)
